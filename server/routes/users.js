...
export default (app) => {
  ...
  app.post('/users', async (req, reply) => {
    console.log('Полученные данные от клиента:', req.body); // Зарегестрированный пользователь

    try {
      const validUser = await app.objection.models.user.fromJson(req.body.data);
      console.log('Проверка валидного пользователя:', validUser); // Проверка полученных данных от пользователя
      ...
      reply
        .code(400)
        .send({
          error: i18next.t('flash.users.create.error'),
          details: error.data || {},
        });
    }
  });
  ...
  app.patch('/users/:id', async (req, reply) => {
    try {
      ...
      reply
        .code(400)
        .send({ error: 'Failed to update user', details: error.data || {} });
    }
  });
};
